...
const Sidebar = ({ isDrawer, isOpen = true, onClose }: SidebarProps) => {
  ...
  return (
    <div className={`${
      isDrawer
        ? `relative w-full h-full p-6 flex flex-col items-center select-none backdrop-blur-2xl border-r ${
            theme === "light" ? "bg-white/95 border-slate-200/80" : "bg-slate-950/95 border-white/10"
          }`
        : `hidden lg:flex flex-col items-center flex-shrink-0 select-none glass-sidebar transition-all duration-500 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] will-change-[width,opacity] ${
            theme === "light" ? "bg-slate-100/55 border-r border-slate-200/60" : "bg-slate-950/50 border-r border-white/[0.04]"
          } ${
      ...
      <div className={`flex flex-col items-center h-full flex-shrink-0 will-change-transform ${
        ...
        <div className="relative group">
          ...
          <div className={`relative w-[150px] h-[150px] lg:w-[170px] lg:h-[170px] rounded-full overflow-hidden border transition-colors ${
            theme === "light" ? "border-slate-200 bg-white" : "border-white/20 bg-slate-950"
          }`}>
            <img
              src={avatar}
              alt="Amamiya Profile"
          ...
          <div className={`absolute bottom-1 right-2 bg-emerald-500 text-white rounded-full p-1.5 border-2 shadow-lg ${
            theme === "light" ? "border-white" : "border-slate-950"
        ...
        <div className="text-center mt-5 w-full">
          ...
          <p className={`text-xs mt-3 max-w-[240px] mx-auto leading-relaxed ${
        ...
        <div className={`w-full mt-6 border-t pt-5 ${
          ...
          <div className="flex flex-wrap justify-center lg:justify-start gap-1.5">
            {["Python", "React", "Linux Admin", "Bash", "Flutter", "AI Agent"].map((badge, idx) => (
              <span
                key={idx}
                className={`text-[9px] font-bold px-2 py-0.5 rounded-md transition duration-200 cursor-default border ${
                  theme === "light"
        ...
        <div className={`w-full mt-6 border-t pt-5 flex flex-col items-center ${
          theme === "light" ? "border-slate-200" : "border-white/[0.04]"
        }`}>
          ...
          <div className="flex gap-2.5 justify-center">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`p-2.5 border rounded-xl hover:-translate-y-1 transition duration-300 shadow ${
                theme === "light"
